...
import { Mail, CheckCircle, Lock, ArrowRight, User, Phone, RefreshCw, Building2, FileText } from 'lucide-react';
...
export default function PartnerRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (cnpjClean.length !== 14) {
      showError('CNPJ deve conter 14 digitos.', 'CNPJ invalido', { label: 'Corrigir', onClick: () => clearFeedback() });
      return;
    }

    if (password !== confirmPassword) {
      showError(
        'As senhas digitadas nao sao iguais. Por favor, verifique e tente novamente.',
        'Senhas nao coincidem',
        { label: 'Corrigir', onClick: () => { clearFeedback(); setConfirmPassword(''); } }
      );
      return;
    }
    ...
    if (password.length < 6) {
      showError(
        'Por seguranca, sua senha precisa ter pelo menos 6 caracteres.',
        'Senha muito curta',
        { label: 'Corrigir', onClick: () => { clearFeedback(); setPassword(''); setConfirmPassword(''); } }
      );
      return;
    }
    ...
    try {
      await registerPartner(email, password, name, cnpj, razaoSocial, phone || undefined);
      setRegistered(true);
    } catch (err: any) {
      setLoading(false);
      const errorMessage = err.message || 'Erro ao criar conta';

      if (errorMessage.toLowerCase().includes('cnpj') && (errorMessage.toLowerCase().includes('exist') || errorMessage.toLowerCase().includes('ja') || errorMessage.toLowerCase().includes('already'))) {
        showError(
          'Ja existe uma conta cadastrada com este CNPJ. Tente fazer login.',
          'CNPJ ja cadastrado',
          { label: 'Ir para login', onClick: () => { clearFeedback(); router.push('/login'); } }
        );
      } else if (errorMessage.toLowerCase().includes('email') && (errorMessage.toLowerCase().includes('exist') || errorMessage.toLowerCase().includes('ja') || errorMessage.toLowerCase().includes('already'))) {
        showError(
          'Ja existe uma conta cadastrada com este email. Tente fazer login ou use outro email.',
          'Email ja cadastrado',
          { label: 'Ir para login', onClick: () => { clearFeedback(); router.push('/login'); } }
        );
      } else {
        showError(errorMessage, 'Erro ao criar conta', { label: 'Tentar novamente', onClick: () => clearFeedback() });
      }
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 flex items-center justify-center p-4">
        ...
        <div className="w-full max-w-md relative">
          ...
          <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 p-8 border border-gray-100 text-center">
            ...
            <p className="text-gray-500 mb-4">Enviamos um link de confirmacao para:</p>
            ...
            <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-2xl p-4 mb-6 border border-emerald-100">
              <div className="flex items-center gap-2 text-emerald-700 text-sm">
                ...
                <span>Clique no link do email para ativar sua conta de parceiro</span>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className={`flex-1 py-4 rounded-2xl font-bold transition-all flex items-center justify-center gap-2 ${
                  resent ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
              >
                {resending ? (
                  ...
                  <><RefreshCw className="w-5 h-5" /> Reenviar</>
                )}
              </button>
              <button
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md relative">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-2">Cadastro de Parceiro Comercial</p>
        </div>
        ...
        <div className="bg-gradient-to-r from-violet-50 to-purple-50 border border-violet-200 rounded-2xl p-4 mb-6 flex items-center gap-3 shadow-lg shadow-violet-100/50">
          ...
          <div>
            <p className="font-semibold text-violet-800">Parceiro PJ</p>
            <p className="text-sm text-violet-600">Ganhe 30% de comissao sobre indicados</p>
          </div>
        </div>

        <form onSubmit={handleSubmit} className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 p-8 border border-gray-100">
          <div className="mb-5">
            <label className="block text-sm font-semibold text-gray-700 mb-2">Razao Social</label>
          ...
          <div className="mb-5">
            <label className="block text-sm font-semibold text-gray-700 mb-2">Nome do responsavel</label>
            <div className="relative">
              <div className="absolute left-4 top-1/2 -translate-y-1/2">
