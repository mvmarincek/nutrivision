  ...
  return (
    <>
      {!isOpen && (
        <button
          onClick={handleOpen}
          className={`fixed bottom-6 right-6 z-50 group ${pulse ? 'animate-bounce' : ''}`}
        >
          <div className="relative">
            <NutraAvatar className="w-16 h-16 drop-shadow-xl group-hover:scale-110 transition-transform" />
        ...
        <div className="fixed bottom-4 right-4 z-50 w-[360px] max-w-[calc(100vw-2rem)] bg-white rounded-2xl shadow-2xl shadow-gray-400/30 border border-gray-100 flex flex-col overflow-hidden" style={{ maxHeight: '500px' }}>
          <div className="bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 px-4 py-3 flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm p-0.5 flex items-center justify-center">
                <NutraAvatar className="w-9 h-9" />
              </div>
              <div>
                <p className="text-white font-bold text-sm">Nutra</p>
                <div className="flex items-center gap-1">
                  <div className="w-2 h-2 rounded-full bg-green-300 animate-pulse" />
                  <span className="text-white/80 text-xs">Sua guia nutricional</span>
                </div>
              </div>
            </div>
            <button
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3 bg-gray-50" style={{ minHeight: '200px' }}>
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start gap-2'}`}>
                {msg.role === 'assistant' && (
                  <NutraAvatar className="w-7 h-7 flex-shrink-0 mt-1" />
                )}
          ...
          <div className="border-t border-gray-100 p-3 bg-white flex-shrink-0">
            <div className="flex items-center gap-2">
              <input
                ...
                placeholder="Pergunte pra Nutra..."
